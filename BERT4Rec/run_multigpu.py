...
class EvalHooks(tf.train.SessionRunHook):
    ...
    def begin(self):
        ...
        if FLAGS.vocab_filename is not None:
            ...
            values = self.vocab.counter.values()   # occure frequency for each word
    ...
    def end(self, session):
        print("EvalHooks end:", self.valid_user)
        print(
...
#               masked_lm_ids.shape, info.shape)
        ...
        for idx in range(len(input_ids)):
            ...
            item_idx = [masked_lm_ids[idx][0]]   # one item index?
            ...
            if FLAGS.use_pop_random:
                if self.vocab is not None:   #choice 100 negative example for each positive example
            ...
            rank = predictions.argsort().argsort()[0]  #first argsort sorts in descending order
...
#         softmax_output_embedding = tf.nn.embedding_lookup(all_user_and_item, item_ids)
        ...
        (masked_lm_loss, masked_lm_example_loss, masked_lm_log_probs) = get_masked_lm_output(
             ...
             masked_lm_weights)   #masked_lm_weights = [1, 1, 1, 1, 1, 0, 0, 0]
...
def input_fn_builder(input_files,
    ...
    def input_fn(params):
        ...
        name_to_features = {
            "info":
            tf.FixedLenFeature([1], tf.int64),  #[user_id]
            "input_ids":
...
def main(_):
    ...
    print("FLAGS.train_input_file", FLAGS.train_input_file)
